...
const Navbar = () => {
  ...
  return (
    <nav
      data-aos="fade-down"
      className="fixed top-3 left-0 w-full flex justify-center items-center px-4 z-[100]"
    >
      <div className="440:px-8 220:px-4 py-1 rounded-xl shadow-2xl bg-white/5 backdrop-blur-xl border border-white/10 hover:shadow-3xl transition-all duration-500 hover:-translate-y-1">
        <div className="p-2 md:p-3">
          <ul className="flex flex-wrap justify-center gap-1 md:gap-6 md:flex-nowrap 220:gap-4">
            {["Home", "Portfolio", "Services", "Contact"].map((item) => (
              <li key={item} className="relative group">
                <a
                  href={`#${item.toLowerCase()}`}
                  className={`text-gray-200 hover:text-cyan-400 text-xs md:text-base font-medium transition-all duration-300 no-underline ${
